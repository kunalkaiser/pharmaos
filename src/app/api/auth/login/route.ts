...
import { findAuthUserByEmail, getUserRoles, markUserLogin } from "@/lib/auth/users";
...
export async function POST(request: NextRequest) {
  ...
  const token = await signAuthSession(
    { sub: user.id, email: user.email, roles, iat: issuedAt, exp: issuedAt + sessionTtlSeconds },
